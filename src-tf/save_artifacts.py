...
def load_config(config_path='config-mac.yaml'):
...
def find_best_checkpoint(checkpoint_dir):
    ...
    # Look for best_model.h5 first (TensorFlow format)
    ...
    # Note: We don't have training_history.json, so use latest checkpoint
...
def create_artifact_bundle(name, output_dir, include_evaluation=True):
    ...
    # Files to copy
    files_to_copy = [
        ("config-mac.yaml", "config.yaml"),
        ("outputs/training/curves/training_curves.png", "training_curves.png"),
    ]
    ...
    if best_checkpoint:
        checkpoint_dst = output_path / "best_model.h5"
        shutil.copy2(best_checkpoint, checkpoint_dst)
        print(f"  ✓ Copied {best_checkpoint.name} -> best_model.h5")
        
        # Also copy the checkpoint info file if it exists
        info_file = best_checkpoint.with_suffix('.h5').with_name(best_checkpoint.stem + '_info.json')
        if info_file.exists():
            info_dst = output_path / "best_model_info.json"
            shutil.copy2(info_file, info_dst)
            print(f"  ✓ Copied {info_file.name} -> best_model_info.json")
    else:
    ...
    metadata = {
        ...
        "config": config,
        "files_included": [dst for _, dst in files_to_copy] + ["best_model.h5"]
    }
    ...
    with open(summary_path, 'w') as f:
        ...
        # Training history summary - note: we don't have training_history.json
        f.write("Training Results:\n")
        f.write(f"  Model saved: {best_checkpoint.name if best_checkpoint else 'None'}\n")
        f.write(f"  Training curves: training_curves.png\n")
        f.write(f"  Configuration: config.yaml\n\n")
        ...
        f.write(f"  Device: {config['training']['device']}\n")
        f.write(f"  Mixed precision: {config['training']['use_amp']}\n")
...
def main():
    ...
    print(f"2. Use the saved config.yaml and best_model.h5")
    print(f"3. Review training_curves.png for performance metrics")
